...
import './IDividexPair.sol';
import './IDividexCallee.sol';
import "@openzeppelin/contracts/token/ERC1155/IERC1155.sol";
import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";
import './IERC20.sol';
import './Math.sol';

contract DividexPair is IDividexPair, ERC1155 {
    ...
    //ERC1155 indexes
    uint public constant indexToken0 = 0;
    uint public constant indexToken1 = 1;
    uint public constant indexLPToken0 = 2;
    uint public constant indexLPToken1 = 3;
    ...
    function getLiabilities() public view returns (uint112  _liability0, uint112 _liability1)  {
        _liability0 = liability0;
        _liability1 = liability1;
    ...
    function _safeTransfer(address token, address to, uint value) private {//this function is made obselete by safeBatchTransfer
    ...
    function _updateSlipDif(uint balance0, uint balance1, uint112 _range0, uint112 _range1,
     uint112 _liability0, uint112 _liability1) private {
        uint balanceModified0 = balance0*(10**9);
        uint balanceModified1 = balance1*(10**9);
    ...
    function mint(address to) external lock returns (uint liquidity0, uint liquidity1) {
        ...
        (uint112 _liability0, uint112 _liability1) = getLiabilities();

        if (_liability0 == 0) {
            liquidity0 = amount0 - MINIMUM_LIQUIDITY;
           _mint(address(0), 2, MINIMUM_LIQUIDITY, ""); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity0 = amount0;
        }

        _mint(to, 2, liquidity0, "");
        

        //token 1 mint determination
        if (_liability1 == 0) {
            liquidity1 = amount1 - MINIMUM_LIQUIDITY;
           _mint(address(0), 3, MINIMUM_LIQUIDITY,""); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity1 = amount1;
        }
        require(liquidity0 > 0 || liquidity1 >0, 'Dividex: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, 3, liquidity0, "");
        
        //why do we not reupdate balances here like in burn - coins have already been added to the pool, look at uint amount0 and amount1
        _updateLiquidity(liquidity0, liquidity1, balance0, balance1); //update slipdif called in this function
        _update(balance0, balance1, _reserve0, _reserve1);
        

        emit Mint(msg.sender, liquidity0, liquidity1);
    }
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        (uint112 _liability0, uint112 _liability1) = getLiabilities(); //gas savings
        ...
        uint liquidity0 = balanceOf(address(this), 2);
        uint liquidity1 = balanceOf(address(this), 3);
        ...
        //_burnBatch(address(this), [2,3], [liquidity0, liquidity1]);
        _burn(address(this), 2, liquidity0);
        _burn(address(this), 3, liquidity0);
        _safeTransfer(_token0, to, amount0); // optimistically transfer tokens
        _safeTransfer(_token1, to, amount1); // optimistically transfer tokens
    ...
    function feeDist(uint fee, bool coin) external lock returns (uint lpReward, uint divReward) {//will be separate for each coin
        ...
        //uint base = (10**9)/2;
        lpReward = fee/2 + fee*Math.sqrt(Math.min(Math.max(slipDif/range,1),0)); //check to make sure each math operation doesn't truncate decimals
        divReward = fee - lpReward;
        require(fee == lpReward + divReward, 'Dividex: Invalid calculation'); //must check out and add to 100%
